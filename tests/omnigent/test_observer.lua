...
T["a reconcile batch writes one recovered header, not one per item"] = function()
  local obs, chat = new_observer()
  obs:reconcile_begin()
  obs:reconcile_item({ id = "m1", type = "message", role = "assistant", content = "A1" })
  obs:reconcile_item({ id = "m2", type = "message", role = "assistant", content = "A2" })
  obs:reconcile_item({ id = "m3", type = "message", role = "assistant", content = "A3" })
  obs:reconcile_end()

  local headers = vim.tbl_filter(function(b)
    return b.type == "sys_msg" and b.content:find("recovered", 1, true) ~= nil
  end, chat.buf_calls)
  h.eq(#headers, 1)
end
